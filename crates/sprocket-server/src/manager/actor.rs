...
use tracing::info;
use uuid::Uuid;
use wdl_cli::execution::execute_workflow;
use wdl_cli::execution::setup_run_dir;
use wdl_cli::execution::WdlSource;
use wdl_engine::Events;

use crate::config::Config;
use crate::db::Database;
use crate::db::WorkflowStatus;
use crate::db::WdlSourceType;
use crate::manager::commands::*;
use crate::names::generate_workflow_name;
...
impl WorkflowManager {
    ...
    async fn handle_submit(&mut self, source: WdlSource, inputs: serde_json::Value) -> Result<SubmitResponse> {
        ...
        let task = tokio::spawn(async move {
            ...
            if let Err(e) = execute_workflow(&workflow_id, &source, &inputs, &run_dir, Events::none(), Some(&db)).await {
...
async fn execute_workflow(
    ...
    let uri = match source {
        WdlSource::Content(content) => {
            ...
            std::fs::write(&wdl_file, content)
                .context("failed to write WDL content to temporary file")?;
    ...
    let workflow_inputs = if inputs.is_null() || inputs.as_object().map_or(false, |o| o.is_empty()) {
        ...
        let inputs_file = temp_dir.path().join("inputs.json");
    ...
    match evaluator
        .evaluate(result.document(), workflow_inputs, &temp_dir)
        .await
    {
        Ok(outputs) => {
            // Serialize outputs.
